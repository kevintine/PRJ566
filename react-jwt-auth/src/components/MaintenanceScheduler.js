...
function MaintenanceScheduler() {
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => {
      ...
      let timeUntilNextTask = ((nextTaskStartTime - currentHour) * 3600) + ((60 - currentMinute) * 60) - currentSecond;
    
      // Adjust time if it's negative (meaning the task is in the next day)
      if (timeUntilNextTask < 0) {
        timeUntilNextTask += 24 * 3600; // Add 24 hours in seconds
      }
    ...
    <div style={styles.container}>
      ...
      {currentTask && (
        <div style={styles.taskContainer}>
          ...
          <p style={styles.countdown}>Next Maintenance in: <span style={{fontSize: '20px', marginRight: '10px'}}>{formatTime(countdown)} hours</span></p>
        </div>
      )}
    </div>
  );
