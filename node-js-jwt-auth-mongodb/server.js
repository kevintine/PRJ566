//PRJ566-MASTER/node-js-jwt-auth-mongodb/server.js
...
const TempUser = require("../node-js-jwt-auth-mongodb/app/models/user.model")

router.post("/saveTempUserScores", async (req, res) => {
  try {
    const users = req.body.users;
    const tempUsers = users.map((user) => new TempUser(user));
    const savedUsers = await TempUser.insertMany(tempUsers);
